<template>
  <p class="text-h6 montserrat-alternates mx-5">Sprites</p>
  <div
    class="text-subtitle-1 d-flex justify-start gap-25 w100 d-flex flex-wrap"
  >
    <img
      v-for="spriteUrl in getSprites"
      :key="spriteUrl"
      class="pokemon-image"
      :src="spriteUrl"
      alt=""
  ...
  >
    ...
    >
      <a
        ...
        @click="emitUpdate(evolution.name)"
      >
        <img class="pokemon-image" :src="spriteUrl" alt="" height="130" />
      </a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async emitUpdate(name) {
      ...
      await this.$router.push({ path: `/pokemon/${name}` });
